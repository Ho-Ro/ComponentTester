...
uint8_t DS18S20_ReadTemperature(int32_t *Value, int8_t *Scale)
{
  ...
  if (Flag)                   /* valid scratchpad */
  {
    ...
    /* standard resolution (1 decimal place, LSB is 0.5캜) */
    ...
    *Value = Temp;            /* save result */
     ...
     *  - 'count per C' is 16
     *  - t_read is is the t value read with the last bit (0.5캜) truncated
     */
...
uint8_t DS18S20_Tool(void)
{
  ...
  while (Run)
  {
    ...
    if (Run)            /* ok to proceed */
    {
      /* get temperature from DS18S20 (in 캜) */
